...
@Component
public class RoleFileParsingStrategy implements FileParsingStrategy {

    private final FileLocationProperties fileLocationProperties;
    private final ObjectMapper objectMapper;

    public RoleFileParsingStrategy(FileLocationProperties fileLocationProperties, ObjectMapper objectMapper) {
    ...
    public boolean supports(Path path) {
        return path != null
                ...
                && hasSegment(path, fileLocationProperties.getRolesDirName());
    }

    @Override
    public LoadedFile parse(Path path) throws IOException {
        if (path == null || !Files.exists(path)) {
            throw new IOException("Role file does not exist: " + path);
        }
        RoleCreationRequest payload = objectMapper.readValue(path.toFile(), RoleCreationRequest.class);
